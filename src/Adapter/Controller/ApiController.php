...
{
    #[Route(path: '/count')]
    public function count(Request $request, LogCounterInterface $logCounter): Response
    {
        $query = $this->getCountQuery($request);
        
        try {
            $count = $logCounter->query($query);
     ...
     * @return CountQuery
     */
    private function getCountQuery(Request $request): CountQuery
    {
        $serviceNames = null;
        $statusCode = null;
        $startDate = null;
        ...
        return new CountQuery($serviceNames, $statusCode, $startDate, $endDate);
    }
}
